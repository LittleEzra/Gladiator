...
public class SteelShotItem extends SlingshotAmmoItem{

    public SteelShotItem(Properties properties) {
        super(properties);
    }

    @Override
    public SlingshotProjectile createProjectile(Level level, ItemStack ammo, LivingEntity shooter, @Nullable ItemStack weapon) {
        return new SteelShot(level, shooter, ammo.copyWithCount(1), weapon);
    }

    @Override
    public Projectile asProjectile(Level level, Position pos, ItemStack stack, Direction direction) {
        return new SteelShot(level, pos.x(), pos.y(), pos.z(), stack.copyWithCount(1), null);
    }
